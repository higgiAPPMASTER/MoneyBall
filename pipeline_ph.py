...
def run_pipeline(run_date: str, emit=None) -> dict:
    ...
    try:
        ...
        pitcher_history_result = await loop.run_in_executor(
            None, lambda: run_pitcher_history(run_date, emit=emit)
        )
    except Exception as exc:
        emit({"type": "log", "msg": f"⚠️ Pitcher History skipped: {exc}"})
        pitcher_history_result = {"pitchers": [], "picks": []}
    ...
    result = {
        ...
        "pitcher_history": pitcher_history_result,
        "stats": {
