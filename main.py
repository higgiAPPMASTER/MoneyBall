...
MIN_AB_7DAY  = 10;   MIN_BA_7DAY  = 0.300   # 7-day hot hitter thresholds
...
def lookup_player(short_name, todays_teams, matchups, pitcher_last=None, date_nodash=None):
    ...
    if pitcher_last and matchups and playing_today:
        try:
            ...
            pass
    # Roster fallback: look up expected batter's team directly
...
# ── 7-Day Hot Hitters (MLB Stats API) ────────────────────────────────────────
def get_hot_hitters_7day(date_str):
    """Fetch all hitters batting .300+ with 10+ AB over the last 7 days."""
    start_str = (datetime.strptime(date_str, "%Y-%m-%d") - timedelta(days=7)).strftime("%Y-%m-%d")
    params = {
        "stats":      "byDateRange",
        "group":      "hitting",
        "gameType":   "R",
    ...
    try:
        resp = requests.get(f"{MLB_API}/stats", params=params, timeout=20)
        resp.raise_for_status()
        data = resp.json()
    except Exception:
        return []
        ...
        for split in stat_group.get("splits", []):
            ...
            if avg >= MIN_BA_7DAY and ab >= MIN_AB_7DAY:
                ...
                players.append({
                    ...
                    "h":         h,
                    "hr":        hr,
                    "step1_ba":  round(avg, 3),
                    "source":    "7day",
                })
    return players

def lookup_player_by_id(player_id):
    """Fast team lookup when player_id is already known from the MLB Stats API."""
    try:
        r = requests.get(f"{MLB_API}/people/{player_id}",
                         params={"hydrate": "currentTeam"}, timeout=10)
        p = r.json()["people"][0]
        return {
            "player_id": player_id,
...
def run_algorithm(date_str, job_id):
    ...
    try:
        ...
        # STEP 1 — FIC matchups + 7-day hot hitters
        upd(1,"Fetching today's matchups from Fantasy Info Central...")
        step1_fic = step1_scrape_fic(date_str)
        for p in step1_fic: p["source"] = "fic"

        upd(1,"Fetching last 7-day hot hitters (BA .300+) from MLB API...")
        step1_hot = get_hot_hitters_7day(date_str)
        for p in step1_hot: p.setdefault("source", "7day")

        # Combine — dedup happens after roster lookup (by player_id)
        step1 = step1_fic + step1_hot
        ...
        if not step1:
            ...
            return
        upd(1, f"Step 1: {len(step1_fic)} FIC + {len(step1_hot)} 7-day hot hitters = {len(step1)} candidates")
        time.sleep(0.3)
        ...
        # Roster lookup — hot hitters use direct ID lookup (faster)
        upd(1,"Looking up player rosters...")
        roster = {}
        for p in step1:
            if p.get("source") == "7day" and p.get("player_id"):
                r = lookup_player_by_id(p["player_id"])
                roster[p["name"]] = r or {
                    "player_id": p.get("player_id"), "full_name": p["full_name"],
                    "slug":      name_to_slug(p["full_name"]),
                    "team_name": "", "team_abbr": ""}
            else:
                pl = p["pitcher"].split(".")[-1].strip() if p.get("pitcher") else ""
        ...
        # ── Dedup by player_id (prefer FIC entry; drop hot-hitter duplicate) ──
        seen_pids = set()
        deduped   = []
        for p in step1:
            pid = (roster.get(p["name"]) or {}).get("player_id")
            if pid and pid in seen_pids:
                continue          # duplicate — already have this player from FIC
            if pid:
                seen_pids.add(pid)
            deduped.append(p)
        # Also drop hot hitters whose team has no game today
        step1 = [p for p in deduped
                 if p.get("source") == "fic" or p.get("team_name") in todays_teams]
        upd(1, f"After dedup + game-day filter: {len(step1)} unique players in pool")
