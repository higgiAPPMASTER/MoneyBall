...
def run_pipeline(run_date: str, emit=None) -> dict:
    ...
    id_map, name_map, teams_with_lineups, rw_lineups, rw_teams = build_lineup_map(run_date)
    n_posted = len(teams_with_lineups | rw_teams)
    emit({"type": "log", "msg": f"✅ Lineup coverage: {len(teams_with_lineups)} MLB confirmed + {len(rw_teams)} Rotowire projected"})
    ...
    try:
        ...
        lineup_data      = (id_map, name_map, teams_with_lineups, rw_lineups, rw_teams)
