...
def _get_7day_hot_hitters(run_date: str, emit) -> list:
    """
    MLB Stats API byDateRange: hitters .300+ BA with 10+ AB in the last 7 days.
    Only includes players whose team is playing today.
    """
    def log(msg):
        if emit: emit({"type": "log", "msg": msg})

    log("⬇️  Source 3: MLB Stats API — 7-day hot hitters (BA >= .300, 10+ AB)...")

    start_str = (_date.fromisoformat(run_date) - timedelta(days=7)).strftime("%Y-%m-%d")
    params = {
        "stats":      "byDateRange",
        "group":      "hitting",
        "gameType":   "R",
    ...
    try:
        ...
        log(f"   ⚠️ 7-day hot hitters fetch failed: {e}")
        return []

    # Build today's team set + pitcher lookup from the schedule
    todays_team_ids = set()
    pitcher_by_team = {}   # team_id -> opposing pitcher short name
    try:
        ...
        for date_data in sched.get("dates", []):
            for game in date_data.get("games", []):
                ...
                home_p = home.get("probablePitcher", {})
                away_p = away.get("probablePitcher", {})
                todays_team_ids.add(home["team"]["id"])
                todays_team_ids.add(away["team"]["id"])
                if home_p:
                    pitcher_by_team[away["team"]["id"]] = _short_name(home_p.get("fullName", ""))
                if away_p:
                    pitcher_by_team[home["team"]["id"]] = _short_name(away_p.get("fullName", ""))
    except Exception as e:
    ...
    for stat_group in data.get("stats", []):
        for split in stat_group.get("splits", []):
            ...
            if avg < 0.300 or ab < 10:
                continue
            if todays_team_ids and team_id not in todays_team_ids:
                continue   # not playing today
            full_name = player.get("fullName", "")
            if not full_name:
                continue
            pitcher = pitcher_by_team.get(team_id, "") if team_id else ""
            results.append({
                "batter":  _short_name(full_name),
                "pos":     pos_abbr,
                "pitcher": pitcher,
                "ab":      ab,
                "h":       h,
    ...
    log(f"✅ Source 3: {len(results)} 7-day hot hitters playing today")
    return results


def _merge_players(mlb_players, bm_players, hot_players=None):
    """Merge 2-3 lists, deduplicating by short batter name (keep highest BA)."""
    merged = {p["batter"]: p for p in mlb_players}
    for p in bm_players:
        name = p["batter"]
        if name not in merged or p["ba"] > merged[name]["ba"]:
            merged[name] = p
    for p in (hot_players or []):
        name = p["batter"]
        if name not in merged or p["ba"] > merged[name]["ba"]:
            merged[name] = p
...
def get_step1_players_or_scrape(run_date=None, min_ab=4, min_ba=0.250, emit=None):
    ...
    log("🔍 Building Step 1 player pool from 3 sources...")
    ...
    # Source 3: MLB Stats API — 7-day rolling (.300+ BA, 10+ AB)
    hot_players = _get_7day_hot_hitters(run_date, emit)

    # Merge all 3
    combined = _merge_players(mlb_players, bm_players, hot_players)

    log(f"✅ Combined pool: {len(combined)} unique players "
        f"({len(mlb_players)} MLB API + {len(bm_players)} hot streaks + {len(hot_players)} 7-day hot, deduped)")
